...
export default function EventDetailPage() {
  ...
  if (error) return <div className="container-content py-24 text-center"><h1 className="font-heading text-3xl font-bold">Event unavailable</h1><p className="mt-3 text-foreground-600">{error}</p><Link to="/events" className="btn-primary mt-7 inline-flex">Back to events</Link></div>;
  if (!event) return <div className="container-content grid min-h-[55vh] place-items-center" role="status"><div className="h-10 w-10 animate-spin rounded-full border-4 border-primary-200 border-t-primary-600" /></div>;
