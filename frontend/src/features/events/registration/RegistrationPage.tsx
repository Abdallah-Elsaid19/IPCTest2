  ...
  return <div className="bg-background-50 py-10 md:py-16">{registrationSeo}<main className="container-content">
    ...
    <form onSubmit={submit} className="grid items-start gap-8 lg:grid-cols-[minmax(0,1fr)_320px]">
      <section className="border border-background-200 bg-white p-6 shadow-lg md:p-9">
        {step === 0 && <div><h2 className="font-heading text-2xl font-bold">Select tickets</h2><p className="mt-2 text-sm text-foreground-600">{event.registration_description || "Choose the number of people attending."}</p><div className="mt-7 flex flex-col gap-5 rounded-xl border border-background-200 p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5"><div><p className="font-bold">{event.registration_title}</p><p className="mt-1 text-sm text-foreground-500">Free</p></div><div className="flex items-center justify-center gap-4 sm:justify-start"><button type="button" onClick={() => setValue("quantity", Math.max(1, quantity - 1))} disabled={quantity <= 1} className="grid h-10 w-10 place-items-center rounded-full border disabled:opacity-40" aria-label="Remove ticket"><Minus size={17}/></button><strong aria-live="polite">{quantity}</strong><button type="button" onClick={() => setValue("quantity", Math.min(maximum, quantity + 1))} disabled={quantity >= maximum} className="grid h-10 w-10 place-items-center rounded-full border disabled:opacity-40" aria-label="Add ticket"><Plus size={17}/></button></div></div>{event.available_places !== null && <p className="mt-3 text-xs text-foreground-500">{event.available_places} place(s) currently available</p>}</div>}
