  ...
  return (
    <div className="fixed inset-0 z-[140] grid place-items-center overflow-y-auto bg-black/60 p-4" role="dialog" aria-modal="true" aria-labelledby="club-join-title" onMouseDown={(event) => { if (event.target === event.currentTarget && !saving) onClose(); }}>
      <section className="my-8 w-full max-w-2xl overflow-hidden rounded-[22px] border border-[#DED2C3] bg-[#FFFDF9] shadow-[0_28px_90px_rgba(0,0,0,.3)]">
        <header className="flex items-start justify-between border-b border-[#E5DACD] px-7 py-6 md:px-8">
        ...
        <form onSubmit={submit}>
          <div className="grid gap-5 px-7 py-7 md:px-8">
          ...
          <footer className="flex justify-end gap-3 border-t border-[#E5DACD] px-7 py-5 md:px-8">
            <button type="button" disabled={saving} onClick={onClose} className="h-12 rounded-xl border border-[#D7C9B8] bg-white px-6 text-sm font-black hover:bg-[#F7F1E9] disabled:opacity-50">Cancel</button>
            <button disabled={saving} className="inline-flex h-12 min-w-44 items-center justify-center gap-2 rounded-xl bg-primary-500 px-6 text-sm font-black text-[#171411] hover:bg-primary-600 disabled:cursor-not-allowed disabled:opacity-55">{saving && <LoaderCircle className="animate-spin" size={17} />}{saving ? "Submitting…" : "Submit request"}</button>
          </footer>
        </form>
      </section>
    </div>
