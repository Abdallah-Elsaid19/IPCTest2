...
import { Check, ChevronLeft, ChevronRight, LoaderCircle, Printer, ShieldCheck } from "lucide-react";
...
import { bursaryApi, pathwayLabels, type BursarySubmissionResponse } from "./api";
import {
  clearBursaryDraft,
  loadBursaryDraft,
  saveBursaryDraft,
...
const stepTitles = [
  ...
  "Bursary Request and Outcomes",
  "Pathway Selection",
  "Mandatory Terms and Consents",
  "Review, Declaration and Signature",
];
...
function ApplicationProgress({
  ...
  return (
    <nav aria-label="Application progress" className="rounded-2xl border border-background-300 bg-white p-4 shadow-sm md:p-5">
  ...
  return (
    <div className="bursary-print-page min-h-[75vh] bg-background-50 px-6 py-16 md:py-24">
      <SEO
        ...
        noIndex
      />
      <div className="bursary-print-summary mx-auto max-w-3xl rounded-3xl border border-background-300 bg-white p-7 text-center shadow-sm md:p-12">
        <div className="mx-auto grid h-16 w-16 place-items-center rounded-full bg-emerald-100 text-emerald-800">
          <ShieldCheck size={30} />
        </div>
        <p className="mt-5 text-xs font-semibold uppercase tracking-[0.16em] text-primary-800">Application received</p>
        ...
        <div className="mx-auto mt-8 max-w-md rounded-2xl border border-primary-300 bg-primary-50 p-6">
        ...
        <div className="mt-8 rounded-2xl border border-background-300 bg-background-100 p-5 text-left">
          ...
          <dl className="mt-4 grid gap-4 text-sm sm:grid-cols-2">
            ...
            <div><dt className="text-xs text-foreground-500">Pathway</dt><dd className="font-medium">{pathwayLabels[values.pathwaySelection.preferredPathway]}</dd></div>
            ...
            <div><dt className="text-xs text-foreground-500">Status</dt><dd className="font-medium">Submitted</dd></div>
          </dl>
        </div>
        <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row print:hidden">
          <button type="button" onClick={() => window.print()} className="btn-primary rounded-xl">
            <Printer size={16} /> Print or save summary
          </button>
          <Link to="/scholarships" className="inline-flex min-h-12 items-center justify-center rounded-xl border border-background-300 px-5 text-sm font-semibold hover:border-primary-500">
...
export default function BursaryApplicationPage() {
  ...
  }, [completedSteps, currentStep, methods]);

  const currentComponent = useMemo(() => {
    if (currentStep === 0) return <PersonalDetailsStep />;
    if (currentStep === 1) return <OrganisationDetailsStep />;
    if (currentStep === 2) return <BursaryRequestStep />;
    if (currentStep === 3) return <PathwaySelectionStep />;
  ...
  const goNext = async () => {
    ...
    if (currentStep === 0) {
      ...
      try {
        const verification = await bursaryApi.validateMembershipReference(
          methods.getValues(fieldName),
        );
        if (verification.authenticated && !verification.valid) {
          methods.setError(fieldName, {
            type: "membershipReferenceOwnership",
            message: verification.message || "This membership reference is not linked to the account currently signed in.",
          }, { shouldFocus: true });
          focusErrorSummary();
          return;
        }
  ...
  const submit = async (values: BursaryApplicationFormValues) => {
    ...
    setSubmitError("");
    try {
      const result = await bursaryApi.submit(values);
  ...
  return (
    <div className="bg-background-50 pb-20 pt-14 md:pt-20">
      <SEO
        ...
        description="Apply for an IPC learner bursary and scholarship pathway."
        canonicalPath="/bursary-scholarship-application"
        noIndex
      />
      <div className="container-content">
        ...
        <div className="mx-auto mt-10 max-w-5xl">
          ...
          />
          <div
            className={`mt-3 flex items-center justify-center gap-2 rounded-xl border px-4 py-2.5 text-xs ${
          ...
          <FormProvider {...methods}>
            <form
              className="mt-6 rounded-3xl border border-background-300 bg-background-50 p-5 shadow-sm md:p-9"
              onSubmit={handleFormSubmit}
              noValidate
            >
              {(submitError || Object.keys(methods.formState.errors).length > 0) && (
                <div
                  ref={errorSummaryRef}
                  tabIndex={-1}
                  role="alert"
                  className="mb-6 rounded-xl border border-red-300 bg-red-50 p-4 text-sm text-red-800 outline-none focus:ring-2 focus:ring-red-500"
                >
              ...
              <div className="sticky bottom-0 z-20 -mx-5 mt-10 flex items-center justify-between gap-3 border-t border-background-300 bg-background-50/95 px-5 py-4 backdrop-blur md:static md:mx-0 md:bg-transparent md:px-0 md:pb-0">
                <button
                  ...
                  className="inline-flex min-h-12 items-center gap-2 rounded-xl border border-background-300 bg-white px-5 text-sm font-semibold disabled:cursor-not-allowed disabled:opacity-40"
                >
                  <ChevronLeft size={17} /> Back
                </button>
                {currentStep < 5 ? (
                  <button type="button" onClick={() => void goNext()} className="btn-primary min-h-12 rounded-xl px-5">
                  ...
                  <button
                    ...
                    className="btn-primary min-h-12 rounded-xl px-5 disabled:cursor-not-allowed disabled:opacity-60"
                  >
