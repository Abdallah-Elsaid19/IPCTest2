...
export default function ContributeImpact(){return <section id="impact" className="scroll-mt-16 bg-background-50 section-padding"><div className="container-content"><div className="reveal grid gap-8 lg:grid-cols-[1fr_.55fr] lg:items-end"><SectionHeader eyebrow="From funding to professional impact" title="Make every contribution purposeful, visible and accountable." subtitle="The strongest funding route begins with a clear audience and ends with a credible professional outcome—not a vague promise."/><div className="lg:text-right"><Link to={informationSessionPath} className="btn-ghost">Design an impact route<i className="ri-arrow-right-line" /></Link></div></div><div className="mt-12 grid border-l border-t border-background-300 sm:grid-cols-2 lg:grid-cols-3">{outcomes.map(([title,copy,icon],index)=><article key={title} className="group min-h-64 border-b border-r border-background-300 p-6 transition-colors hover:bg-background-100 md:p-8"><div className="flex items-center justify-between"><span className="font-mono text-[10px] font-bold tracking-widest text-primary-600">0{index+1}</span><i className={`${icon} text-xl text-primary-600`} /></div><h3 className="mt-10 text-xl font-semibold text-background-950">{title}</h3><p className="mt-4 text-sm leading-[1.75] text-foreground-600">{copy}</p></article>)}</div></div></section>}
