  ...
  return (
    <>
      <section className="relative flex min-h-[72vh] items-center overflow-hidden bg-background-950 pb-20 pt-24 text-background-50">
        <div className="absolute inset-0"><ResponsiveImage src="/images/membership/hero.svg" alt="" width={1600} height={900} sizes="100vw" priority className="h-full w-full object-cover opacity-25" /></div>
        ...
        <div className="container-content relative z-10 grid items-center gap-14 pt-12 lg:grid-cols-[1.05fr_.95fr]">
          <div className="reveal max-w-3xl">
            <span className="eyebrow text-primary-400">IPC funded opportunities</span>
            <h1 className="mt-5 font-heading text-5xl font-bold leading-[1.05] sm:text-6xl lg:text-7xl">Fund the people behind <span className="text-primary-400">better project decisions.</span></h1>
            <p className="mt-7 max-w-2xl text-base leading-[1.8] text-background-200 md:text-lg">Support future project-controls talent, professional learning, regional capability and applied research across planning, cost, risk, change, data and assurance.</p>
            <div className="mt-9 flex flex-wrap gap-3"><a href="#applicant-route" className="btn-primary">Apply for support</a><a href="#funder-route" className="btn-secondary">Fund an opportunity</a></div>
            ...
            <dl className="mt-10 grid border-y border-background-50/15 sm:grid-cols-3">{heroPoints.map(([title, copy], index) => <div key={title} className={`py-5 sm:px-5 ${index > 0 ? "border-t border-background-50/15 sm:border-l sm:border-t-0" : "sm:pl-0"}`}><dt className="text-sm font-semibold text-primary-300">{title}</dt><dd className="mt-1 text-xs leading-relaxed text-background-400">{copy}</dd></div>)}</dl>
          </div>
