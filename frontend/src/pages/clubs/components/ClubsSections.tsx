          ...
          <div className="mt-14 grid border border-background-300 bg-background-50 lg:grid-cols-[minmax(15rem,.34fr)_1fr]">
            ...
            <div role="tabpanel" className="min-w-0">
              ...
              <aside className="flex flex-col gap-7 border-t border-background-800 bg-background-950 p-7 text-background-50 sm:flex-row sm:items-center sm:justify-between md:px-10 md:py-8 lg:px-12">
                ...
                <div className="shrink-0"><ProtectedButton onAction={onAction}>{selected.cta_label}</ProtectedButton></div>
              </aside>
            </div>
          </div>
        )}
...
export function ClubsProgramme({ onAction }: { onAction: ProtectedAction }) {
  ...
  return <section className="bg-background-950 section-padding"><div className="container-content"><SectionHeader eyebrow={intro.eyebrow} title={intro.title} subtitle={intro.description} light centered /><div className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{items.map((item) => <button type="button" onClick={onAction} key={item.id} className="text-left"><FeatureCard icon={item.icon} title={item.title} description={item.description} light /></button>)}</div></div></section>;
}
...
export function ClubsAudiences({ onAction }: { onAction: ProtectedAction }) {
  ...
  return <section className="bg-background-50 section-padding"><div className="container-content"><SectionHeader eyebrow={intro.eyebrow} title={intro.title} subtitle={intro.description} centered /><div className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{items.map((item) => <article key={item.id} className="flex min-h-80 flex-col border border-background-200 border-t-2 border-t-primary-500 p-7"><i className={`${item.icon} text-2xl text-primary-600`} /><h3 className="mt-8 font-heading text-xl font-semibold text-background-950">{item.title}</h3><p className="mt-4 text-sm leading-relaxed text-foreground-600">{item.description}</p><button type="button" onClick={onAction} className="mt-auto pt-7 text-left text-sm font-semibold text-primary-700">{item.cta_label} →</button></article>)}</div></div></section>;
}
...
export function ClubsPartners({ onAction }: { onAction: ProtectedAction }) {
  ...
  const items = activeItems(content.items ?? []);
  return <section className="bg-background-50 section-padding"><div className="container-content"><SectionHeader eyebrow={content.eyebrow} title={content.title} subtitle={content.description} centered /><div className="mt-14 grid gap-6 lg:grid-cols-3">{items.map((item: any) => <article key={item.id} className="flex min-h-96 flex-col border border-background-200 border-t-2 border-t-primary-500 p-7"><h3 className="font-heading text-xl font-semibold">{item.title}</h3><ul className="mt-6 space-y-3 text-sm text-foreground-600">{item.benefits?.map((benefit: string) => <li key={benefit} className="flex gap-3"><i className="ri-check-line text-primary-600" />{benefit}</li>)}</ul><button type="button" onClick={onAction} className="mt-auto pt-8 text-left font-semibold text-primary-700">{item.cta_label} →</button></article>)}</div><p className="mt-7 border-l-2 border-primary-500 px-5 py-3 text-xs text-foreground-600">{content.notice}</p></div></section>;
}
