    ...
    <section className="bg-background-50 section-padding">
      <div className="container-content">
        <div className="reveal mx-auto max-w-4xl text-center">
          ...
          <h2 className="font-heading text-4xl font-semibold leading-tight text-background-950 md:text-5xl">
            {content.title}
          </h2>
          <p className="mx-auto mt-6 max-w-3xl leading-relaxed text-foreground-600">
            {content.description}
          </p>
        </div>

        <div className="mx-auto mt-12 max-w-4xl space-y-3 md:mt-16">
          {questions.map(({ question, answer }, index) => {
            ...
            return (
              <article
                key={question}
                className="border border-background-300 bg-background-50"
              >
                <button
                  type="button"
                  onClick={() => setOpenQuestion(isOpen ? -1 : index)}
                  aria-expanded={isOpen}
                  className="flex w-full items-center justify-between gap-5 p-5 text-left font-semibold text-background-950"
                >
                  <span>{question}</span>
