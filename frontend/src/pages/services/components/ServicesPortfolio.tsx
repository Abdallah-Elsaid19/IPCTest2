...
export default function ServicesPortfolio() {
  ...
  const section = useManagedSection("portfolio", {
    ...
    items: serviceRoutes.map((route) => ({...route, tab_description: route.tabDescription, next_step: route.nextStep, enquiry_title: route.enquiryTitle, enquiry_description: route.enquiryDescription, cta_label: route.cta, cta_url: "/information-session"})),
  ...
  const routes = section.items.filter(isManagedItemActive);
  const active = routes.find((service) => service.id === activeId) ?? routes[0];
            ...
            {routes.map((service) => {
              ...
              return (
                <button key={service.id} id={`service-tab-${service.id}`} type="button" role="tab" aria-selected={selected} aria-controls="service-panel" onClick={() => setActiveId(service.id)} className={`min-h-24 border-b border-background-200/80 p-4 text-left transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-primary-500 xl:border-b-0 xl:border-r ${selected ? "bg-background-950 text-background-50" : "hover:bg-background-100"}`}>
                  ...
                  <span className={`mt-1 block text-xs ${selected ? "text-background-400" : "text-foreground-500"}`}>{service.tab_description}</span>
                </button>
              );
            })}
          </div>
          <div id="service-panel" role="tabpanel" aria-labelledby={`service-tab-${active.id}`} className="grid lg:grid-cols-[1.16fr_0.84fr]">
            <div className="p-6 md:p-9 lg:p-10">
              ...
              <dl className="mt-8 grid gap-3 md:grid-cols-3">
                {[["Best for", active.audience], ["Core value", active.value], ["Next step", active.next_step]].map(([label, value]) => (
            ...
            <aside className="flex flex-col justify-between bg-background-950 p-6 text-background-50 md:p-9 lg:p-10">
              <div><span className="eyebrow text-primary-300">Service enquiry</span><h3 className="mt-3 text-xl font-semibold">{active.enquiry_title}</h3><p className="mt-4 text-sm leading-relaxed text-background-300">{active.enquiry_description}</p><p className="mt-6 border border-background-50/15 bg-background-50/5 p-4 text-xs leading-relaxed text-background-400">{active.note}</p></div>
              <Link
                to="/information-session"
                state={{ enquiry: active.enquiry_title, service: active.tab }}
                className="btn-primary mt-8 w-fit"
              >
                {active.cta_label} <i className="ri-arrow-right-line" aria-hidden="true" />
              </Link>
            </aside>
          </div>
        </div>
        <p className="mt-5 border-l-2 border-primary-500 pl-4 text-xs leading-relaxed text-foreground-500">{section.notice}</p>
      </div>
    </section>
  );
}
