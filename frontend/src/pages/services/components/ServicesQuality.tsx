...
export default function ServicesQuality() {
  const section = useManagedSection("quality", {
    ...
    items: standards.map(([title, description]) => ({title, description})),
  });
  return (
    <section className="bg-background-50 section-padding">
      <div className="container-content grid gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:gap-16">
        <div className="reveal flex min-h-[400px] flex-col justify-between bg-accent-700 p-7 text-background-50 md:p-10"><div><span className="eyebrow text-primary-300">{section.eyebrow}</span><h2 className="mt-4 font-heading text-3xl font-semibold leading-tight md:text-4xl">{section.title}</h2><p className="mt-5 text-sm leading-relaxed text-background-200 md:text-base">{section.description}</p></div><Link to={section.cta_url} className="btn-primary mt-8 w-fit">{section.cta_label}</Link></div>
        <dl className="reveal grid gap-3">{section.items.filter(isManagedItemActive).map(({title, description}) => <div key={title} className="grid gap-2 border border-background-200/80 bg-background-50 p-5 sm:grid-cols-[150px_1fr]"><dt className="text-xs font-bold uppercase tracking-wider text-primary-700">{title}</dt><dd className="text-sm leading-relaxed text-foreground-600">{description}</dd></div>)}</dl>
      </div>
    </section>
  );
}
